...
int ispower2(int n){
    ...
    // we have also noticed that all powers of 2 have only 1 set bit like 8= 1000 or 16=100000 or 4=0100 so if we and them with n-1 we will get 0 always 
    return (n && !(n & n-1));
...
int to_find_no_of_ones(int n){
    int count=0;
    while(n!=0){
        n=(n&n-1); // here n & n-1 keeps removing the 1's
        count++;
    }
    return count;
}
